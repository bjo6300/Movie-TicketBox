...
public class PremiumScreen extends Screen {
	...
	public void reserveTicket() { // 좌석 예약
		...
		int rownum, colnum;
		while (true) {
			System.out.print("좌석 행 번호 입력(1~5) : ");
			rownum = scan.nextInt();
			System.out.print("좌석 열 번호 입력(1~5) : ");
			colnum = scan.nextInt();
	...
	public void changeTicket() { // 
		...
		while (true) {
			...
			if(seatArray[rownum][colnum].getcStatus()!=MovieTicket.SEAT_EMPTY_MARK) {
				System.out.println("예약된 좌석입니다.");
				break;
			}
			...
			System.out.println("예약번호 "+existrv.getnReservedId()+" 행[" + rownum + "] 열[" + colnum + "] " + "좌석으로 변경되었습니다.");
	...
	public void payment() {
		...
		while(true) {
			...
			if(way == Pay.BANK_TRANSFER_PAYMENT) { //은행 이체
				...
				System.out.println("은행 결제가 완료 되었습니다. : "+nTicketPrice*(1+BankTransfer.BANK_TRANSFER_COMMISION)+"원");
